...
2. Login with service principal
   azd auth login --tenant-id xxxxxx --client-id xxxx --client-secret xxxx --no-prompt (slalom)
   azd auth login --tenant-id xxxx --client-id xxxx --client-secret xxxx --no-prompt (personal)
